...
let retryDuration = 2; // Retry in 15 seconds by default
...
function validateConfig() {
    ...
    if (rpc.assets.largeImageText && rpc.assets.largeImageText.length > 100) {
    ...
    if (rpc.assets.smallImageText && rpc.assets.smallImageText.length > 100) {
    ...
    if (rpc.buttons.primary.url) {
        if (rpc.buttons.primary.url.length > 0 && rpc.buttons.primary.url.length <= 100) {
            ...
            console.error(ERROR(`Primary button url provided exceeds the maximum character length of 100.`));
            return process.exit(0)
        }
    }
    ...
    if (rpc.buttons.secondary.url) {
        if (rpc.buttons.secondary.url.length > 0 && rpc.buttons.secondary.url.length <= 100) {
            ...
            console.error(ERROR(`Secondary button url provided exceeds the maximum character length of 100.`));
            return process.exit(0)
        }
    }
